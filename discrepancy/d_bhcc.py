...
class D_BHCC(object):
    ...
    def forward(self, source, target, nums_S, nums_T):
        assert(len(nums_S) == len(nums_T)), "The number of domains for source (%d) and target (%d) should be the same." % (len(nums_S), len(nums_T))

        num_domains = len(nums_S)
        ...
        for i in range(self.num_layers):
            for c in range(num_domains):
                gamma_layers[i]['ss'][c] = gamma_layers[i]['ss'][c].view(num_domains, 1, 1)
                gamma_layers[i]['tt'][c] = gamma_layers[i]['tt'][c].view(num_domains, 1, 1)
        ...
        kernel_dist_tt = []
        for c in range(num_domains):
            kernel_dist_ss += [torch.mean(self.kernel_layer_aggregation(dist_layers, gamma_layers, 'ss', c).view(num_domains, -1), dim=1)] # dist_layers['ss']:3x3，gamma_layers['ss']:num_domains x1x1
            kernel_dist_tt += [torch.mean(self.kernel_layer_aggregation(dist_layers, gamma_layers, 'tt', c).view(num_domains, -1), dim=1)]
        ...
        d_hcc_row = max_intra_value - min_inter_from_row
        d_hcc_col = max_intra_value - min_inter_from_col
        d_bhcc = (d_hcc_row + d_hcc_col) / 2.0
